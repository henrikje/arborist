...
__arb_complete_list() {
    ...
    COMPREPLY=($(compgen -W "--fetch -N --no-fetch --no-status -q --quiet -d --dirty -w --where --older-than --newer-than --json --schema" -- "$cur"))
}
...
__arb_complete_reset() {
    ...
    if [[ "$cur" == -* ]]; then
        COMPREPLY=($(compgen -W "--fetch -N --no-fetch --base --soft --mixed --hard -y --yes -n --dry-run --include-wrong-branch -w --where" -- "$cur"))
        return
    fi
    COMPREPLY=($(compgen -W "$(__arb_workspace_repo_names "$base_dir")" -- "$cur"))
}
