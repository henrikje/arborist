...
describe("replayPlan annotation", () => {
  ...
  async function setupDivergedWithCherryPick(env: {
    ...
    // The independent commit ensures the cherry-pick gets a different hash than feature1
    // (different parent chain), creating a true divergence detectable by patch-id.
    const tmpClone = join(env.testDir, "tmp-cherry");
  ...
  test("arb log --json includes replayPlan when branch is diverged with cherry-picked commits", () =>
    withEnv(async (env) => {
      await arb(env, ["create", "my-feature", "repo-a"]);
      ...
      expect(repoA.replayPlan.contiguous).toBe(true);
    }));

  test("arb log does not include replayPlan when branch is not diverged", () =>
    withEnv(async (env) => {
      await arb(env, ["create", "my-feature", "repo-a"]);
      await write(join(env.projectDir, "my-feature/repo-a/file.txt"), "change");
      await git(join(env.projectDir, "my-feature/repo-a"), ["add", "file.txt"]);
  ...
  test("arb log --json omits replayPlan for non-diverged repos in same workspace", () =>
    withEnv(async (env) => {
      await arb(env, ["create", "my-feature", "repo-a", "repo-b"]);
      await setupDivergedWithCherryPick(env);
