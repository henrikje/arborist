...
**Never call `process.exit()` directly in commands or library code.** Throw `ArbError` (exit 1) or `ArbAbort` (exit 130). Single try/catch in `index.ts` maps types to exit codes.
...
**Always call `error()` before throwing `ArbError`.** The catch handler in `index.ts` does not print the exception message — it only maps the type to an exit code. Throwing without logging first produces a silent exit 1.

**Safety over power.** Nothing mutates without explicit approval. `--yes` skips the confirmation prompt; `--force` overrides safety checks — never combine them into one flag.

**Always read [GUIDELINES.md](GUIDELINES.md) before making changes.** It covers design principles, command conventions, the status model, mutation flow, output and rendering, error handling, git internals, and commit conventions.
...
- See GUIDELINES.md § Mutation Flow for rationale and the shared-logic override exposure rule.
...
- **`status/`** — Canonical status model: `status.ts` (RepoStatus, RepoFlags, gathering, filtering — see GUIDELINES.md § Status Model), `skip-flags.ts`, `pr-detection.ts`, `ticket-detection.ts`, `test-helpers.ts` (makeRepo fixtures)
...
- **Guidelines** — If the change introduces a new convention or modifies an existing one, update the relevant section of GUIDELINES.md.
