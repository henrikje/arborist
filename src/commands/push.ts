...
export function buildPushPlanNodes(
  ...
  if (mergedNewWorkCount > 0) {
    nodes.push({
      kind: "hint",
      cell: cell(
        `  hint: ${plural(mergedNewWorkCount, "repo")} ${mergedNewWorkCount === 1 ? "was" : "were"} merged but ${mergedNewWorkCount === 1 ? "has" : "have"} new commits since — run 'arb rebase' or 'arb extract --from-merge <workspace>' to split`,
        "muted",
      ),
    });
  }
