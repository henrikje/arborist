...
export function registerRebaseCommand(program: Command, getCtx: () => ArbContext): void {
	program
		...
		.option("-F, --fetch", "Fetch from all remotes before rebase (default)")
		.option("--no-fetch", "Skip fetching before rebase")
